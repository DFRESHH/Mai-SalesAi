  ...
  if (!isExecuting) {
    ...
    const executeTrade = async (_exchangePath, _token0, _token1, _amount) => {
      ...
      try {
        ...
        const transaction = await arbitrage.connect(account).executeTrade(routerPath, tokenPath, POOL_FEE, _amount);
        const receipt = await transaction.wait();
    
        console.log(`Trade executed successfully. Transaction Hash: ${receipt.transactionHash}`);
      } catch (error) {
        console.error(`Trade execution failed: ${error.message}`);
        console.error(`Failed Transaction Details: Router Path - ${routerPath}, Token Path - ${tokenPath}, Amount - ${_amount.toString()}`);
      }
    };
...
const executeTrade = async (_exchangePath, _token0, _token1, _amount) => {
  ...
  if (config.PROJECT_SETTINGS.isDeployed) {
    const increasedAmount = _amount.mul(1.1); // Increase trade size by 30%
